			...
			<div class="span8">
				...
				<div class="portlet-body scroll-area">
					<div id="scroller-chat" class="scroller" data-height="200px" data-spy="scroll" data-always-visible="1" data-rail-visible="1">
						...
						@else
						{{ Form::model($data['chat-with'], array('method' => 'PATCH', 'class' => 'form-horizontal', 'route' => array('keuangan.chat.update', $data['chat-with']))) }}
						...
						<div style="border-bottom: solid 1px #eeeeee;">
							<table>
								<tr>
									...
									@if( $mychat->user1 == $id )
									<td>
										<a href="{{ URL::asset('keuangan/chat/'.$mychat->receiver->username) }}"><h4>{{ $mychat->receiver->username }}
									...
									@else
									<td>
										<a href="{{ URL::asset('keuangan/chat/'.$mychat->sender->username) }}"><h4>{{ $mychat->sender->username }}
											@if( array_sum($stat[$i]) > 0 )
												<span class="badge badge-important">{{ array_sum($stat[$i]) }}</span>
											@endif
										</h4>
		...
		<div style="border-bottom: solid 1px #eeeeee;">
			<table>
				<tr>
					...
					<td>
						<a href="{{ URL::asset('keuangan/chat/'.$user->username) }}"><h4>{{ $user->username }}</h4></a>
						<p>{{ $user->nama }}</p>
					</td>
				</tr>
			</table>
